from classless_methods import reverse_complement, calculate_degeneracy, calculate_GC, calculate_end_stats, \
    calculate_longest_monorun, calculate_longest_duorun, calculate_degeneracy_with_segment
...
class Primer:
    ...
    def check_feasibility(self, comparison_matrix, gc_lb=0.4, gc_ub=0.6, melting_lb=55, melting_ub=75, end_at_threshold=2, end_gc_threshold=3, monorun_threshold=3, duorun_threshold=2, mfe_threshold=-5, self_complementarity_threshold=10, verbose=False):
        ...
        if verbose:
            problems = []
            self.degeneracy, segment = calculate_degeneracy_with_segment(self.sequence)
            if segment:
                problems.append("Primer is part of 2 different segments")
            ...
            degeneracy, segment = calculate_degeneracy_with_segment(self.sequence)
            if segment:
                self.feasible = False
                return False
            if degeneracy > 1:
